...
class Paragraph:
    ...
    def toBlock(self, index: int):
        return SRTBlock(index=index, start=self.startTime, end=self.endTime, content=self.text)
    ...
    while index < len(real_bytes):
        ...
        if paragraph is not None and len(paragraph.text) > 0:
            if len(all_pars) == 0:
                ...
                all_pars.append(paragraph.toBlock(index=len(all_pars) + 1))
        index += 1
...
def getTimeCode(timeCodeIndex, byte_list):
    ...
    if timeCodeIndex > 0:
        ...
        return TimeCode(str(hours).rjust(2, "0"), str(minutes).rjust(2, "0"), str(seconds).rjust(2, "0"),
...
def getPacParagraph(index, real_bytes, codePage):
    ...
    while index < len(real_bytes) and index <= maxIndex:
        if preTextCode == 'W16':
            ...
            raise LanguageNotSupported('Not currently supported')
...
def main(file_path, codePage=None):
    usage = "usage: python readPac.py [options] pac_file"
    file_type = file_path.split('.')[-1]
    ###Work out encoding & Read File
    if codePage:
        codePage = codePage.lower()
        paragraphs = loadSubtitle(file_path, codePage)
    elif file_type.lower() == 'fpc':
        paragraphs = loadSubtitle(file_path, 'utf-8')
    else:
        paragraphs = autoDetect(file_path)

    return paragraphs


if __name__ == "__main__":
    d = main('/Users/yovel.c/PycharmProjects/services/sublineStreamlit/samples/TR778638.pac')
    print(d)
