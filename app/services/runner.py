...
from services.format_handlers import SRTHandler, JSONHandler, QTtextHandler, BaseHandler


def get_handler_by_mime_type(
        mime_type: Literal['xml', 'json', 'srt', 'qtext'] | None = None, blob_content: str = None
) -> Type[BaseHandler]:
    """
    Returns a handler class based on the mime type of the file
    :param mime_type:
    :param blob_content:
    """
    if not mime_type:
...
async def run_translation(
    ...
    """
    if mime_type == 'xml':  # TODO: Until re implementing XMLHandler
        blob_content = xml_to_srt(blob_content)
    handler = get_handler_by_mime_type(mime_type=mime_type, blob_content=blob_content)
